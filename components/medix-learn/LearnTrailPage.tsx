import type { LearnTrailData } from "@/lib/medix-learn/types";
...
function Divider() {
  return <hr style={{ border: "none", borderTop: "1px solid rgba(120,130,180,0.14)", margin: 0 }} />;
}

export default function LearnTrailPage({ trail, systemId }: Props) {
  const systemLabel = SYSTEM_LABELS[systemId] ?? systemId;
  const systemHref = `/learn/${systemId}`;

  return (
    <LearnShell>
      <LearnHero
        titulo={trail.titulo}
        subtitulo={trail.subtitulo}
        badges={trail.badges}
        breadcrumb={[
          { label: "MEDIX Learn", href: "/learn" },
          { label: systemLabel, href: systemHref },
          { label: trail.titulo, href: "#" },
        ]}
      />

      <div style={{ padding: "10px 16px", borderRadius: 10, background: "rgba(241,245,249,0.9)", border: "1px solid rgba(120,130,180,0.14)", fontSize: 12, color: "#5c6d8a" }}>
        <span style={{ fontWeight: 700, color: "#0b1f4d" }}>Público-alvo: </span>
        {trail.publicoAlvo}
      </div>

      {trail.objetivos.length > 0 && (
      ...
      {trail.secoes.length > 0 && (
        <>
          <SectionTitle>Conteúdo</SectionTitle>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {trail.secoes.map((sec, i) => (
      ...
      {trail.questoesAtivas.length > 0 && (
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div>
              <SectionTitle>Questões ativas</SectionTitle>
              <p style={{ margin: "4px 0 0", fontSize: 13, color: "#5c6d8a" }}>Clique para revelar a resposta esperada.</p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {trail.questoesAtivas.map((q, i) => (
      ...
      {trail.mapaLinhas.length > 0 && (
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <SectionTitle>Mapa final</SectionTitle>
            <LearnMapBox linhas={trail.mapaLinhas} />
          </div>
          <Divider />
        </>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#5c6d8a" }}>Aplique o que aprendeu na prática simulada.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 14 }}>
          {trail.pontes.map((ponte) => (
