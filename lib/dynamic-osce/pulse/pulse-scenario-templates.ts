...
export const PULSE_SCENARIO_TEMPLATES: PulseScenarioTemplate[] = [
  ...
  },
  {
    ...
    compatibility: "strong",
    recommendedProvider: "hybrid",
    initialPhysiology: { fc: 108, fr: 30, paSys: 145, paDia: 85, spo2: 84, temp: 37.8, resumo: "Retenção de CO₂, roncos/sibilos, musculatura acessória, SpO₂ 84%." },
    requiredActions: ["oxigenio_controlado", "salbutamol", "ipratropio", "corticoide"],
    dangerousDelays: ["oxigenio_alto_fluxo_sem_controle", "sedativo_sem_indicacao"],
    pulseScenarioCandidates: [`${P}/patient/COPDExacerbation.json`],
    medixOverlayRequired: true,
    rubricImplications: ["Alvo SpO₂ 88–92%; oxigenio_controlado obrigatório; hiperóxia = erro crítico."],
  },
  {
    ...
    compatibility: "strong",
    recommendedProvider: "hybrid",
    initialPhysiology: { fc: 118, fr: 32, paSys: 105, paDia: 65, spo2: 87, temp: 39.1, resumo: "Consolidação em base direita, febre, hipoxemia, taquipneia, risco de sepse." },
    requiredActions: ["oxigenio_suplementar", "antibiotico_precoce", "antitermico", "hidratacao_cautelosa", "monitorizacao", "reavaliar", "internacao"],
    dangerousDelays: ["alta_precoce", "atrasar_antibiotico_por_exames"],
    pulseScenarioCandidates: [
      `${P}/patient/Pneumonia.json`,
      `${P}/patient/PneumoniaSevere.json`,
      `${P}/patient/SepsisPneumonia.json`,
    ],
    medixOverlayRequired: true,
    rubricImplications: ["Antibiótico precoce é conduta essencial (efeito não fisiológico imediato no Pulse); atrasar = erro crítico."],
  },
  {
